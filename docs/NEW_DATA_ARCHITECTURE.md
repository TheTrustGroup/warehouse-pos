...
| **Phase 7** | Wire Realtime to real IDs. useWarehouseRealtime(currentWarehouseId). Remove sentinel subscription. **DONE:** InventoryContext passes warehouseId (= currentWarehouseId) to useInventoryRealtime; hook uses only that ID, no sentinel; filters warehouse_inventory_by_size and sales by warehouse_id; doc and hook comments updated. |
| **Phase 8** | Delete dead code. Remove SENTINEL_EMPTY_WAREHOUSE_ID, PLACEHOLDER_WAREHOUSE_ID, isMainStoreIdWithWrongLabel, rawWarehouseId, effectiveWarehouseId, cacheRef, localStorage product cache, conditional fallback queries. **DONE:** Removed PLACEHOLDER_WAREHOUSE_ID from lib/warehouseId.ts; removed DEFAULT_WAREHOUSE_ID re-export from WarehouseContext; InventoryContext updateProduct no longer uses DEFAULT_WAREHOUSE_ID (sends current warehouseId or ''); renamed effectiveWarehouseId to resolvedWarehouseId in WarehouseContext. cacheRef/localStorage product cache already removed in Phase 6 Part 2. |
