...
- **"Failed to load resource: the server responded with a status of 403 ()"** appearing a little while after refresh: In **Network** tab, find the request that returned 403. If the URL is **ingest…sentry.io**, it’s Sentry (error reporting). To stop the console message: set **`VITE_SENTRY_ENABLED=false`** in your frontend env (Vercel or `.env`), or remove `VITE_SENTRY_DSN`. This does not affect app behaviour. If the URL is your **API** (e.g. `/api/products`, `/api/warehouses`), the app now shows a toast "Access denied (403)…"; fix auth/permissions or Deployment Protection on the API host.
