-- Safely handle p_quantity_by_size when it is a scalar (e.g. string) instead of array.
-- Fixes "cannot get array length of a scalar" when client sent JSON.stringify(array) or single object.
-- Drop by full signature so the name is unique when multiple overloads exist.

-- Argument order must match existing definitions (see pg_get_function_arguments).
drop function if exists create_warehouse_product_atomic(uuid, uuid, jsonb, integer, jsonb);
drop function if exists update_warehouse_product_atomic(uuid, jsonb, uuid, integer, integer, jsonb);
...
declare
  ...
  v_by_size jsonb;
begin
  v_by_size := case when jsonb_typeof(p_quantity_by_size) = 'array' then p_quantity_by_size else '[]'::jsonb end;
  ...
  insert into warehouse_products (
    ...
    case when jsonb_array_length(v_by_size) > 0 then 'sized'
         else coalesce(lower(nullif(trim(p_row->>'size_kind'), '')), lower(nullif(trim(p_row->>'sizeKind'), '')), 'na') end
  );

  if jsonb_array_length(v_by_size) > 0 then
    v_qty := 0;
    for v_entry in select * from jsonb_array_elements(v_by_size)
    loop
...
comment on function create_warehouse_product_atomic is 'Atomic create: product + warehouse_inventory + warehouse_inventory_by_size. Safely handles p_quantity_by_size when not array.';
comment on function update_warehouse_product_atomic is 'Atomic update: product (version check) + warehouse_inventory + warehouse_inventory_by_size. Safely handles p_quantity_by_size when not array.';
