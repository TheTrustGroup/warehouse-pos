-- Audit trail: who completed the sale (email; optional uuid when from Supabase Auth).
-- Add sold_by_email to sales and record_sale so POST /api/sales can pass auth.email.

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'sales' AND column_name = 'sold_by_email'
  ) THEN
    ALTER TABLE sales ADD COLUMN sold_by_email text;
  END IF;
END $$;

COMMENT ON COLUMN sales.sold_by_email IS 'Email of the user who completed the sale (cashier/admin).';

-- Recreate record_sale with p_sold_by_email. Signature: add one optional param.
