-- Derive quantity from actual data: use by_size sum when present, else inv.quantity.
-- Fixes dashboard showing "out of stock" for products that have size-level stock but
-- warehouse_products.size_kind was not 'sized' (so RPC previously used inv.quantity = 0).
...
COMMENT ON FUNCTION get_warehouse_inventory_stats(uuid) IS
  'Returns one row: total_stock_value, total_products, total_units, low_stock_count, out_of_stock_count. Quantity derived from by_size sum when present, else warehouse_inventory.quantity.';
