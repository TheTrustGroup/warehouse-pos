...
export default function InventoryPage({ apiBaseUrl: _ignored }: InventoryPageProps) {
  ...
  const loadInflightRef   = useRef(false); // prevents duplicate simultaneous fetches
  const currentWarehouse  = WAREHOUSES.find(w => w.id === warehouseId) ?? WAREHOUSES[0];

  const { toasts, show: showToast } = useToast();

  // ── apiFetch with retry ────────────────────────────────────────────────────
  // Vercel cold starts can cause the first request to take 5-8 seconds and
  // fail. We retry GET requests up to 3 times with exponential backoff.
  // Non-GET requests (POST/PUT/DELETE) are NOT retried — they're not idempotent.
  ...
  const apiFetch = useCallback(async <T = unknown>(
    ...
    const timeoutMs  = 20_000; // 20s — generous for cold starts
    ...
    try {
      ...
      clearTimeout(timeout);
      // Retry on network errors and timeouts (GET only, max 3 tries)
      const isNetworkErr = e.name === 'AbortError' || e.name === 'TypeError' || e.message === 'Failed to fetch';
  ...
  // Deduplication: if a load is already in flight, skip silents, queue non-silents
  const loadProducts = useCallback(async (silent = false) => {
    if (modalOpenRef.current) return;
    // Drop concurrent silent refreshes — already loading
    ...
    try {
      ...
      if (!silent) setError(e.message ?? 'Failed to load products');
    } finally {
  ...
  function closeModal() {
    ...
    // Silent refresh after modal closes — picks up server truth
  ...
  async function handleSubmit(
    ...
    if (isEdit && payload.id) {
      ...
      try {
        const raw = await apiFetch<unknown>(`/api/products/${payload.id}`, {
          ...
          body: JSON.stringify({
            ...payload,
            warehouseId,
            sizeKind:       payload.sizeKind,        // always explicit
            quantityBySize: payload.quantityBySize,  // always explicit
            quantity:       payload.quantity,         // always explicit
          }),
        });
        // Apply server response (ground truth) over our optimistic update
      ...
      try {
        ...
        const created = unwrapProduct(raw);
        if (created?.id) {
          // Prepend to list immediately — shows up without waiting for next poll
          setProducts(prev => [created, ...prev]);
        } else {
          // Server returned unexpected shape — do a fresh load to sync
